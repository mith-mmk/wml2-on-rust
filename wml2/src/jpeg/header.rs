...
impl JpegHaeder {
    ...
    pub(crate) fn dht_read<B: BinaryReader>(reader:&mut B,huffman_tables:&mut HuffmanTables) ->  Result<(),Error> {
        ...
        while size < length {
            ...
            let tc = t >> 4;
            let th = t & 0x0f;
            let ac = if tc == 0 { false } else if tc == 1 { true } else {
                let boxstr = format!("Huffman table class require 0 or 1 but {}",tc);
                return Err(Box::new(
                    ImgError::new_const(
            ...
            if ac {
                if huffman_tables.ac_tables[no].is_none() {
                    huffman_tables.ac_tables[no] = Some(HuffmanTable::new(ac,no,len,p,val))
                }
            } else {
                if huffman_tables.dc_tables[no].is_none() {
    ...
    pub(crate) fn dqt_reader<B: BinaryReader>(reader:&mut B,quantization_tables:&mut Vec<QuantizationTable>) -> Result<(),Error> {
    ...
    pub(crate) fn read_makers<B: BinaryReader>(reader:&mut B,opt :usize,include_soi:bool,is_only_tables:bool) ->  Result<Self,Error> {
        ...
        if _sof_flag && _sos_flag && _dht_flag && _dqt_flag == false {
            return Err(Box::new(ImgError::new_const(ImgErrorKind::IllegalData,"Maker is shortage".to_string())));
        }
