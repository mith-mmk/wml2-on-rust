...
def main():
    ...
    for source in sorted(Path(args.corpus).iterdir()):
        ...
        try:
            with warnings.catch_warnings():
                ...
                with Image.open(source) as image:
                    ...
                    supported = (compression in (1, 2, 3, 4, 5, 7, 8, 32946, 32773)
        ...
        result = dict(name=source.name, **info)
        try:
            ...
            if converted.returncode == 0:
                ...
                if len(outputs) != 1:
                    ...
                    reference, actual = pixels(source), pixels(outputs[0])
                    result["oracle"] = "ImageMagick"
                    if info.get("compression") == 7:
                        # LibTIFF's RGBA path in ImageMagick can misinterpret JPEG strips.
