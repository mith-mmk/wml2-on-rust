...
- 全25件がconverterでPNG化できます。12-bit sampleはdecode可能ですが、FFmpegとの最終RGB差分は平均約41.77で、strict pixel oracleは未達です。`avis` の primary item は先頭フレームを静止画として変換できます。
...
成功25件は `supported/` または既存の外部sample置き場と `converted/avif/` にあります。strict pixel oracle未達の12-bit sampleを含む全件を再確認するときは、
上記スクリプトを実行します。
