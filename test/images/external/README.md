...
- 現在の互換性ゲートは34件をPNG化できます。12-bit sampleもFFmpegのRGB oracleを通過し、最終RGB差分は平均約0.075、最大6です。`avis` の primary item は先頭フレームを静止画として変換できます。
- `alpha_noispe.avif` は CDEF/SGRPROJ の entropy restoration parity が未実装のため、`too many padding bits` を返し、部分PNGを生成しない期待失敗として固定しています。
...
| unsupported | [alpha_noispe.avif](https://raw.githubusercontent.com/AOMediaCodec/libavif/main/tests/data/alpha_noispe.avif) | 取得時固定 | `8deb96e78c3e5d608a157b2de4c98eb1a30e0c85736b4230758400509c88d47e` | BSD-2-Clause / AOMediaCodec | reduced still、CDEF/SGRPROJ restoration | 期待失敗: `too many padding bits`、部分PNGなし |
