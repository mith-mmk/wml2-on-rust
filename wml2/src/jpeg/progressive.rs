...
pub fn decode_progressive<'decode,B: BinaryReader>(reader:&mut B,header: &JpegHaeder,option:&mut DecodeOptions,mut warnings:Option<ImgWarnings>) -> Result<Option<ImgWarnings>,Error> {
    ...
    let mut huffman_tables = header.huffman_tables.as_ref().unwrap().clone();
        ...
        for mcu_y in 0..mcu_y_max {
            for mcu_x in 0..mcu_x_max {
                ...
                for scannumber in 0..mcu_size {
                    ...
                    if ss == 0 {
                        let pred = preds[i];
                        zz[0] = dc_read(&mut bitread, &dc_decode[dc_current], pred)? << al as usize;
                        preds[i] = zz[0];
                        if se == 0 {
                            continue;
                        }
                    ...
                    if ah == 0  {
                        eobrun = progressive_ac_read(&mut bitread, &ac_decode[ac_current],zz,start,se,ah,al,eobrun)?;
                    } else {
                        eobrun = successive_approximation_read(&mut bitread, &ac_decode[ac_current],zz,start,se,ah,al,eobrun)?;
                    }
                }
            ...
            match b {
                Ok(marker) => {
                    match marker {
                        ...
                        0xc4 => { // DHT
                            JpegHaeder::dht_read(bitread.reader, &mut _huffman_tables)?;
                            huffman_tables = &_huffman_tables;
                        },
                        0xda => { // SOS 
...
fn progressive_ac_read<B: BinaryReader>(bitread:&mut BitReader<B>, ac_decode:&HuffmanDecodeTable,zz:&mut [i32],ss: usize,se: usize,_ah: usize,al: usize,mut eob: usize)
    ...
    while zigzag <= se {
        ...
        if ssss == 0 {
            ...
            zz[zigzag] = extend(v,ssss as usize) << al as usize;
        }
        if zigzag >= se {
            return Ok(0)
        }
    ...
    while zigzag <= se {
        ...
        while i <= se {
            if zz[i] == 0 {
                ...
                rrrr -= 1;
            } else if bitread.get_bit()? == 1 &&  zz[i] & val == 0 {
                zz[i] += val;
            } else {
                zz[i] -= val;
            }
