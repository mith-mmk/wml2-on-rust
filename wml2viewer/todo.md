...
3. Issueの修正, ファイラー：表示が遅い問題
4. Issueの修正, ZIP：二枚目以降が表示されない場合がある
5. Issueの修正, ZIP：FitScreenなどの指示が無視されるケースがある
6. Issueの修正,マンガモード:表示が中央にアラインされない（右端にアラインされる）
7. Issueの修正,Viewer:ワークアラウンド：フルスクリーンモードで起動すると表示がおかしいので。起動時は無効化
8. todo.mdの整理 todo.mdと実装を比較して終了しているタスクには[+]を付ける
9.  ファイラー：一覧表示、サムネイル表示（大・中・小）、詳細表示、の追加と切り替えボタン
10. ファイラー：UIにメタデータも表示出来る様にして、ソートボタンにする
11. 設定：閉じるボタンの実装
12. CTRL+S でファイル保存（保存用ディレクトリを選べる様にする）
13. 外部プラグインの実装：susie plugin の実装とplugin conifigの実装
    1. プラグインの有効化
