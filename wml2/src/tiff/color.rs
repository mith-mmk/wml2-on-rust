...
pub(crate) fn unassociate_alpha(r: u8, g: u8, b: u8, alpha: u8) -> (u8, u8, u8) {
    ...
    let unassociate = |value: u8| {
        (u16::from(value) * 255 + u16::from(alpha) / 2)
            .checked_div(u16::from(alpha))
            .unwrap_or(255)
            .min(255) as u8
    };
    (unassociate(r), unassociate(g), unassociate(b))
}
...
pub(crate) fn gray_to_rgba8(
    ...
    let (red, green, blue) = if associated {
        unassociate_alpha(value, value, value, alpha)
    } else {
        (value, value, value)
    };
    RGBA {
